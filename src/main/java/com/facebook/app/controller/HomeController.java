...
public class HomeController {
	...
	public ResponseEntity<?> viewUser(@PathVariable String user_facebook_id, Model model) throws IOException {
		// i don't need model here for this task , but i will leave it and maybe
		// implement some view latter
		Users user = userDao.findById(user_facebook_id);
		if (user == null) {
			return new ResponseEntity(new CustomErrorType("User " + user_facebook_id + " Not found"),
	...
	@RequestMapping(value = "/users/{facebookId}/photos", method = RequestMethod.GET)
	public ResponseEntity<?> viewphotos(@PathVariable String facebookId, Model model) throws IOException {
		
		List<Photos> photo = new ArrayList<Photos>();
		photo = userDao.findPhotosById(facebookId);
		if (photo == null) {
			return new ResponseEntity(new CustomErrorType("Photo with userID " + facebookId + " Not found"),
					HttpStatus.NOT_FOUND);
		}
	...
	public ResponseEntity<?> deleteUser(@PathVariable(value = "user_fb_id") String user_fb_id) {
		...
		if (user == null) {
			...
			return new ResponseEntity(
					new CustomErrorType("Unable to delete. User with facebookID " + user_fb_id + " not found."),
					HttpStatus.NOT_FOUND);
		}
